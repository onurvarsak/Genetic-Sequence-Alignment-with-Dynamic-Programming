...
def cal(Opt,i=0,j=0):
    if i < M and i >=0:
        if j < N and j >=0:
            if X[i] == Y[j]:
                Opt[i][j] = min( cal(Opt,i,j+1)[i][j+1]+2, cal(Opt,i+1,j)[i+1][j]+2, cal(Opt,i+1,j+1)[i+1][j+1]+0 )
                return Opt 
            else:
                Opt[i][j] = min( cal(Opt,i,j+1)[i][j+1]+2, cal(Opt,i+1,j)[i+1][j]+2, cal(Opt,i+1,j+1)[i+1][j+1]+1 )
                return Opt
        else:
            return Opt
    else:
